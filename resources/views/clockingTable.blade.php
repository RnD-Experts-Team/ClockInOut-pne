                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach($clockings as $clocking)
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        <button type="button"
                                            class="edit-btn inline-flex items-center px-3 py-1 border border-transparent
                                                   text-sm leading-4 font-medium rounded-md text-white bg-blue-600
                                                   hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2
                                                   focus:ring-blue-500"
                                            data-clocking-id="{{ $clocking->id }}"
                                            data-clock-in="{{ $clocking->clock_in
                                                ? \Carbon\Carbon::parse($clocking->clock_in)->format('Y-m-d\TH:i')
                                                : '' }}"
                                            data-clock-out="{{ $clocking->clock_out
                                                ? \Carbon\Carbon::parse($clocking->clock_out)->format('Y-m-d\TH:i')
                                                : '' }}"
