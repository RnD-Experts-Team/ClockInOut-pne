    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add-another-shift').on('click', function() {
                ...
                $template.find('.shift-data').attr('data-shift-id', shiftId).val(JSON.stringify({ start, end, type: shiftType, role, color, task_id: taskId }));
            ...
            // Delete shift
            // Delete shift
// Delete shift - COMPLETE CORRECTED VERSION
            $(document).on('click', '.delete-shift', function(e) {
                e.stopPropagation(); // Prevent cell click event
                console.log('🔴 Delete Shift Clicked - Starting deletion process');
                ...
                console.log(`🔍 Shift details:`, {
                    shiftId, taskId, employeeId, date, shiftStart, shiftEnd
                });

                // Add to deletedShiftIds if it's a real database ID
                if (shiftId && !shiftId.toString().startsWith('temp_')) {
                    deletedShiftIds.add(parseInt(shiftId));
                    console.log(`✅ Added shift ID ${shiftId} to deletedShiftIds`);
                    console.log(`Current deletedShiftIds:`, Array.from(deletedShiftIds));
                }

                // Remove from assignedTasks
                if (taskId) {
                    assignedTasks.delete(parseInt(taskId));
                    console.log(`✅ Removed taskId ${taskId} from assignedTasks`);
                }

                // Remove from scheduleData
                const possibleKeys = [
                    `${employeeId}_${date}`,
                    `${employeeId}_${date} 00:00:00`
                ];
                ...
                if (targetKey && window.scheduleData[targetKey]) {
                    console.log(`🔍 Found schedule data for key: ${targetKey}`);
                    console.log(`🔍 Before deletion:`, window.scheduleData[targetKey]);

                    const originalLength = window.scheduleData[targetKey].length;
                    ...
                    window.scheduleData[targetKey] = window.scheduleData[targetKey].filter(shift => {
                        ...
                        if (shift.id && shiftId && !shiftId.toString().startsWith('temp_')) {
                            const match = shift.id == shiftId;
                            if (match) {
                                console.log(`🗑️ Removing shift with ID ${shift.id}`);
                            }
                            return !match;
                        }
                        // Match by time and task for temp shifts
                        else {
                            const timeMatch = shift.start === shiftStart && shift.end === shiftEnd;
                            const taskMatch = shift.task_id == taskId;
                            const match = timeMatch && taskMatch;
                            if (match) {
                                console.log(`🗑️ Removing temp shift: ${shift.start}-${shift.end}, task ${shift.task_id}`);
                            }
                            return !match;
                        }
                    });

                    console.log(`🔍 Removed ${originalLength - window.scheduleData[targetKey].length} shift(s)`);
                    console.log(`🔍 After deletion:`, window.scheduleData[targetKey]);

                    // If no shifts left, remove the key entirely
                    if (window.scheduleData[targetKey].length === 0) {
                        delete window.scheduleData[targetKey];
                        console.log(`🗑️ Removed empty key: ${targetKey}`);
                    }

                    // Update the UI - either show remaining shifts or clear the cell
                    ...
                    console.log(`⚠️ No schedule data found for employee ${employeeId} on ${date}`);
                    console.log(`Available keys:`, Object.keys(window.scheduleData));
                }

                // Update available tasks
                filterAvailableTasks();

                console.log('🟢 Delete Shift Process Completed');
                console.log(`🔍 Final deletedShiftIds:`, Array.from(deletedShiftIds));
            });
            // Save shift
            // Save shift - LOCAL ONLY (Don't submit form)
            $('#save-shift').on('click', function() {
                const employeeId = $('#modal-employee-id').val();
                const date = $('#modal-date').val();
                let shifts = [];

                // Get existing shifts to preserve IDs
                ...
                // Preserve ID if this is an existing shift
                ...
                $('#additional-shifts-container .additional-shift').each(function() {
                    ...
                    // Preserve ID if this corresponds to an existing shift
                ...
                // Update scheduleData LOCALLY ONLY
                ...
                console.log('✅ Shift saved locally (not submitted to server yet)');
            ...
            $('#update-schedule').on('click', function(e) {
                ...
                const scheduleArray = Object.entries(window.scheduleData).map(([key, shifts]) => {
                    ...
                    return {
                        ...
                        start_date: date.split(' ')[0], // Remove time part if present
                        shifts
                    };
                });
                ...
                // Add deleted shift IDs
                console.log('🔍 Preparing form with deletedShiftIds:', Array.from(deletedShiftIds));
                ...
                if (deletedShiftIds.size > 0) {
                    ...
                    console.log(`✅ Set deleted_shift_ids to: ${deletedIds}`);
                }
