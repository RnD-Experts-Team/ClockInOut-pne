...
@section('content')
        ...
        <div class="bg-orange-50 shadow-lg rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-orange-200">
                <div class="flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-black-900">Edit Payment</h2>
            ...
            <form action="{{ route('payments.update', $payment) }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="bg-orange-100 rounded-lg p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center">
                                <input type="radio" name="store_option" value="existing" class="form-radio text-black-600 focus:ring-orange-500" {{ $payment->store_id ? 'checked' : '' }}>
                            ...
                            <label class="flex items-center">
                                <input type="radio" name="store_option" value="manual" class="form-radio text-black-600 focus:ring-orange-500" {{ !$payment->store_id ? 'checked' : '' }}>
                                <span class="ml-2 text-sm text-black-700">Enter Store Manually</span>
                            </label>
                        </div>
                    </div>

                    <!-- Existing Store Selection -->
                    <div id="existing-store-section" class="mb-4 {{ !$payment->store_id ? 'hidden' : '' }}">
                        ...
                        <select name="store_id" id="store_id" class="form-select block w-full rounded-lg border-orange-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm @error('store_id') border-red-500 @enderror">
                            ...
                            @if(isset($stores))
                                @foreach($stores as $store)
                                    <option value="{{ $store->id }}" {{ old('store_id', $payment->store_id) == $store->id ? 'selected' : '' }}>
                    ...
                    <!-- Manual Store Entry -->
                    <div id="manual-store-section" class="mb-4 {{ $payment->store_id ? 'hidden' : '' }}">
                        <label for="store" class="form-label block text-sm font-medium text-black-700 mb-2">Store Number</label>
                        <input type="text" name="store" id="store"
                               class="form-field block w-full border-orange-300 rounded-lg shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm @error('store') border-red-300 @enderror"
                               value="{{ old('store', $payment->store) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="maintenance_type" id="maintenance_type"
                               ...
                               value="{{ old('maintenance_type', $payment->maintenance_type) }}"
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              ...
                              placeholder="Additional notes or invoice links...">{{ old('notes', $payment->notes) }}</textarea>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const manualStore = document.getElementById('store');
            ...
            function toggleStoreOptions() {
                ...
                if (manualStore) manualStore.required = false;
                ...
                if (selectedOption === 'existing') {
                    ...
                    if (manualStore) manualStore.required = true;
                }
            }
