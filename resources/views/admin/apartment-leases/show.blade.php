    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-700">{{ Str::limit($apartmentLease->apartment_address, 80) }}</p>
            </div>
        ...
        @if($apartmentLease->expiration_date)
            ...
            @if($showAlert)
                <div class="mb-6 p-4 rounded-lg {{ $isExpired ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }}">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @if($apartmentLease->store_number)
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->store_number }}</dd>
                            </div>
                        @endif
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Drive Time</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->drive_time ?? 'N/A' }}</dd>
                        </div>
                        <div class="md:col-span-2">
                            <dt class="text-sm font-medium text-gray-500">Apartment Address</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->apartment_address }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            ...
                            <dd class="mt-1 text-lg font-semibold text-blue-600">${{ number_format($apartmentLease->rent, 2) }}</dd>
                        </div>
                        @if($apartmentLease->utilities)
                            <div class="text-center p-4 bg-green-50 rounded-lg">
                                <dt class="text-sm font-medium text-gray-500">Monthly Utilities</dt>
                                <dd class="mt-1 text-lg font-semibold text-green-600">${{ number_format($apartmentLease->utilities, 2) }}</dd>
                            </div>
                        @endif
                    </div>
                    <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                        <div class="flex justify-between items-center">
                            <span class="text-lg font-medium text-gray-900">Total Monthly Cost:</span>
                            <span class="text-xl font-bold text-green-600">${{ number_format($apartmentLease->total_rent, 2) }}</span>
                        </div>
                    </div>
                    <div class="mt-4 p-4 bg-purple-50 rounded-lg">
                        <div class="flex justify-between items-center">
                            <span class="text-lg font-medium text-gray-900">Annual Cost:</span>
                            <span class="text-xl font-bold text-purple-600">${{ number_format($apartmentLease->total_rent * 12, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            <dt class="text-sm font-medium text-gray-500">Number of AT</dt>
                            <dd class="mt-1 text-lg font-semibold text-blue-600">{{ $apartmentLease->number_of_AT }}</dd>
                        </div>
                        <div class="text-center p-4 bg-orange-50 rounded-lg">
                            <dt class="text-sm font-medium text-gray-500">Number of Cars</dt>
                            <dd class="mt-1 text-lg font-semibold text-orange-600">{{ $apartmentLease->has_car }}</dd>
                        </div>
                        <div class="text-center p-4 {{ $apartmentLease->is_family === 'Yes' || $apartmentLease->is_family === 'yes' ? 'bg-green-50' : 'bg-gray-50' }} rounded-lg">
                            <dt class="text-sm font-medium text-gray-500">Family Unit</dt>
                            <dd class="mt-1">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $apartmentLease->is_family === 'Yes' || $apartmentLease->is_family === 'yes' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <dl class="grid grid-cols-1 gap-6">
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Lease Holder(s)</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->lease_holder }}</dd>
                        </div>
                        @if($apartmentLease->expiration_date)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Lease Expiration Date</dt>
                ...
                @if($apartmentLease->notes)
                    <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <p class="text-sm text-gray-900 whitespace-pre-wrap">{{ $apartmentLease->notes }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <div class="space-y-4">
                        @if($apartmentLease->expiration_date)
                            ...
                            @elseif($now->diffInDays($expirationDate, false) <= 30)
                                <div class="p-4 bg-yellow-50 rounded-lg">
                                    <h3 class="text-sm font-medium text-yellow-900">Lease Status</h3>
                        ...
                        <div class="p-4 bg-blue-50 rounded-lg">
                            <h3 class="text-sm font-medium text-blue-900">Monthly Rent per AT</h3>
                            <p class="text-lg font-semibold text-blue-700">
                                ${{ number_format($apartmentLease->total_rent / max($apartmentLease->number_of_AT, 1), 2) }}
                            </p>
                            <p class="text-xs text-blue-500">Per AT per month</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('admin.apartment-leases.index') }}"
                           class="w-full inline-flex items-center justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-all duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg p-6">
                    ...
                    <dl class="space-y-4">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->created_at->format('M j, Y g:i A') }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Last Updated</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->updated_at->format('M j, Y g:i A') }}</dd>
                        </div>
                        @if($apartmentLease->created_at != $apartmentLease->updated_at)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Last Modified</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $apartmentLease->updated_at->diffForHumans() }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
                ...
                <div class="bg-white shadow-sm ring-1 ring-red-200 rounded-lg p-6 border-red-200">
                    ...
                    <p class="text-sm text-red-700 mb-4">This action cannot be undone. This will permanently delete the apartment lease record.</p>
