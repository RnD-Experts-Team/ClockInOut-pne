...
<div class="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 py-4 sm:py-8" dir="{{ getDirection() }}">
    <div class="mx-auto max-w-sm sm:max-w-2xl lg:max-w-4xl xl:max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-4 sm:mb-6 space-y-4 rounded-2xl bg-white p-4 sm:p-5 lg:p-6 shadow-lg ring-1 ring-orange-900/5">
            ...
            {{-- ── Add Manual Fix (technician only, card not completed) ── --}}
            @if(Auth::user()->role !== 'admin' && $card->status !== 'completed')
            <div class="rounded-lg mt-4 bg-orange-50 border border-orange-200 p-4">
                <div class="flex items-center justify-between mb-3">
                    <h4 class="text-sm font-semibold text-gray-800">Manual Fix Record</h4>
                    <button type="button"
                            onclick="document.getElementById('manualFixForm').classList.toggle('hidden')"
                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-orange-600 text-white text-xs font-medium rounded-lg hover:bg-orange-700 transition-colors">
                ...
                <div id="manualFixForm" class="hidden">
                    @if(session('success'))
                        <div class="mb-3 p-3 bg-green-100 border border-green-300 text-green-800 text-xs rounded-lg">{{ session('success') }}</div>
                    @endif
                    <form method="POST" action="{{ route('invoice.cards.manual-fix.store', $card->id) }}"
                        ...
                        <div>
                            ...
                            <select name="equipment_id" required
                                    class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:ring-orange-500 focus:border-orange-500">
                                <option value="">Select equipment…</option>
                                @foreach(\App\Models\Equipment::active()->forStore($card->store_id)->orderBy('name')->get() as $eq)
                        ...
                        <div>
                            ...
                            <textarea name="description_of_issue" rows="2" required
                                      class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:ring-orange-500 focus:border-orange-500"
                                      placeholder="What was the problem?">{{ old('description_of_issue') }}</textarea>
                            @error('description_of_issue')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">How We Fixed It</label>
                            <textarea name="how_we_fixed_it" rows="2"
                                      class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:ring-orange-500 focus:border-orange-500"
                                      placeholder="Optional — describe the fix">{{ old('how_we_fixed_it') }}</textarea>
                            @error('how_we_fixed_it')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="file" name="before_image" accept="image/*"
                                       class="w-full text-xs text-gray-700 border border-gray-300 rounded-lg cursor-pointer bg-white
                                              file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs
                                              file:font-medium file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100">
                                @error('before_image')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">
                                    <span class="inline-flex items-center gap-1">
                                ...
                                <input type="file" name="after_image" accept="image/*"
                                       class="w-full text-xs text-gray-700 border border-gray-300 rounded-lg cursor-pointer bg-white
                                              file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs
                                              file:font-medium file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                                @error('after_image')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                        </div>

                        <div class="flex gap-2">
                            <button type="submit"
                                    class="px-4 py-2 bg-orange-600 text-white text-xs font-medium rounded-lg hover:bg-orange-700 transition-colors">
                                Add to Card
                            </button>
                            <button type="button"
                                    onclick="document.getElementById('manualFixForm').classList.add('hidden')"
                                    class="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-200 transition-colors">
                                Cancel
                            </button>
                        </div>
                    </form>
